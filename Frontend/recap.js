function recapCommande(data) {
	...
	var title = document.getElementById('orderId');
	title.appendChild(text);

	article.setAttribute('id', 'recap');
	if (nbrOfArticles == 1) {
		article.innerHTML += `<p class="recap">Merci pour votre confiance!<br>
        <br> votre commande a bien été enregistrée et transmise au vendeur pour validation: nombre de commande: ${JSON.parse(
			localStorage.product
		).length} numéro de commande: ${orderId} vous sera livrée dans les plus brefs délais.
        ...
        <br> Nombre de commande: ${JSON.parse(localStorage.product)
			.length} le numéro du commande ${orderId} vos commandes seront livrées dans les plus brefs délais.
        <br> Votre commande a pour total: ${total}
